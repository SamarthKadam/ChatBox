  ...
  return (
    <div className="max-w-[60%]">
      <div className="flex flex-row justify-start my-1">
        ...
        <div className="bg-[#FFFFFF] max-w-[100%]  rounded-tr-xl ml-[1%] font-Roboto rounded-br-xl rounded-bl-xl box-border px-2 py-2 flex flex-col items-end max-[900px]:text-sm flex justify-between">
          <p className="w-[100%]" style={{wordWrap:"break-word"}}>{content}</p>
          <p className=" text-[11px] pl-2 pt-1 flex items-end font-medium">{`${String(messageTime.getHours() % 12 || 12).padStart(2, '0')}:${String(messageTime.getMinutes()).padStart(2, '0')} ${messageTime.getHours() >= 12 ? 'pm' : 'am'}`}</p>
        </div>
      </div>
    </div>
  );
